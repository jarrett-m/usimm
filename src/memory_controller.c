...
void init_memory_controller_vars() {
  ...
  for (int i = 0; i < NUM_CHANNELS; i++) {
    ...
    for (int d = 0; d < DOMAIN_COUNT; d++) {
      domain_queues[i][d] = NULL;
      secure_read_queue_length[i][d] = 0;
      secure_write_queue_length[i][d] = 0;
    }
...
void *init_new_node(long long int physical_address, long long int arrival_time,
                    ...
                    long long int instruction_pc, int domain_id) {
...
int read_matches_write_or_read_queue_secure(long long int physical_address, int domain_id) {
  // get channel info
  dram_address_t *this_addr = calc_dram_addr(physical_address);
  int channel = this_addr->channel;
  free(this_addr);
  ...
  LL_FOREACH(domain_queues[channel][domain_id], wr_ptr) {
    if (wr_ptr->dram_addr.actual_address == physical_address) {
      num_read_merge++;
      stats_reads_merged_per_channel[channel]++;
      return WQ_LOOKUP_LATENCY;
  ...
  LL_FOREACH(domain_queues[channel][domain_id], rd_ptr) {
    if (rd_ptr->dram_addr.actual_address == physical_address) {
      num_read_merge++;
      stats_reads_merged_per_channel[channel]++;
      return RQ_LOOKUP_LATENCY;
...
int write_exists_in_write_queue_secure(long long int physical_address, int domain_id) {
  // get channel info
  dram_address_t *this_addr = calc_dram_addr(physical_address);
  int channel = this_addr->channel;
  free(this_addr);
...
request_t *insert_read(long long int physical_address,
  ...
  request_t *new_node =
      init_new_node(physical_address, arrival_time, this_op, thread_id,
                    instruction_id, instruction_pc, 0);
...
//insert into channels custom domain queues
request_t *insert_write_secure(long long int physical_address,
                        long long int arrival_time, int thread_id,
                        int instruction_id, int domain_id) {
  optype_t this_op = WRITE;
  ...
  request_t *new_node = init_new_node(physical_address, arrival_time, this_op,
                                      thread_id, instruction_id, 0, domain_id);

  LL_APPEND(domain_queues[new_node->dram_addr.channel][domain_id], new_node);

  secure_write_queue_length[new_node->dram_addr.channel][domain_id]++;
...
request_t *insert_read_secure(long long int physical_address,
                       long long int arrival_time, int thread_id,
                       int instruction_id, long long int instruction_pc,
                       int domain_id) {
  ...
  request_t *new_node =
      init_new_node(physical_address, arrival_time, this_op, thread_id,
                    instruction_id, instruction_pc, domain_id);

  LL_APPEND(domain_queues[new_node->dram_addr.channel][domain_id], new_node);

  secure_read_queue_length[new_node->dram_addr.channel][domain_id]++;
...
// Similar to update_read_queue above, but for secure queue
void update_entire_queue_commands_secured(int channel){
  request_t *curr = NULL;

  for(int domain = 0; domain < DOMAIN_COUNT; domain++){
    LL_FOREACH(domain_queues[channel][domain], curr) {
      if (curr->request_served == 1)
        continue;
      ...
      case ACTIVE_POWER_DOWN:
        ...
        if (forced_refresh_mode_on[channel][rank] && curr->operation_type == WRITE)
          curr->command_issuable = 0;
...
void clean_queues(int channel) {
  ...
  for(int doms = 0; doms < DOMAIN_COUNT; doms++){
    LL_FOREACH_SAFE(domain_queues[channel][doms], rq_ptr, rq_tmp){
      if(rq_ptr->request_served == 1){
        assert(rq_ptr->next_command == COL_WRITE_CMD || rq_ptr->next_command == COL_READ_CMD);

        LL_DELETE(domain_queues[channel][doms], rq_ptr);

        if(rq_ptr->operation_type == WRITE){
          secure_write_queue_length[channel][doms]--;
          assert(secure_write_queue_length[channel][doms] >= 0);
        }

        if(rq_ptr->user_ptr)
          free(rq_ptr->user_ptr);
...
void update_memory() {
  for (int channel = 0; channel < NUM_CHANNELS; channel++) {
    ...
    if (SECURED == 0){
      ...
      update_entire_queue_commands_secured(channel);
    }
...
*/
    ...
    /*
     ...
     printf("%3d %11d %16.2f %16.2f %17.2f %13.2f %13.2f %20.2f %21.2f %24.2f
     %12.2f %13.2f\n",\
                                    channel,\
                                    rank,\
                                    total_rank_power, \
    ...
    */
    ...
    /*
                    printf("Channel:%d Rank:%d Total background power : %f
       mW\n", channel, rank,
       psch_act_pdn+psch_act_stby+psch_pre_pdn_slow+psch_pre_pdn_fast+psch_pre_stby);
                    printf("Channel:%d, Rank:%d Total activate power :
       %f,%f,%d,%f mW\n", channel, rank,
       psch_act,pds_act,T_RC,average_gap_between_activates[channel][rank]);
                    printf("Channel:%d, Rank:%d Total I/O and termination power:
       %f rd:%f wr:%f dq:%f termW:%f termRoth:%f termWoth:%f mW\n", channel,
       rank, psch_rd+psch_wr+psch_dq+psch_termW+psch_termRoth+psch_termWoth,
       psch_rd, psch_wr, psch_dq, psch_termW, psch_termRoth, psch_termWoth);

                    printf("Channel:%d, Rank:%d Total refresh power: %f mW\n",
       channel, rank, psch_ref); printf("Channel:%d, Rank:%d Total Rank power:
       %f mW\n\n", channel, rank, total_rank_power);
                    printf("------------------------------------------------\n");
                    */
