...
int main(int argc, char *argv[]) {
  ...
  domain_id = (int *)malloc(sizeof(int) * NUMCORES);
  ...
  for (int i = 0; i < NUMCORES; i++) {
    ...
    ROB[i].domain_id = (int *)malloc(sizeof(int) * ROBSIZE);
  }
  ...
  for (numc = 0; numc < NUMCORES; numc++) {
    if (fgets(newstr, MAXTRACELINESIZE, tif[numc])) {
      if (sscanf(newstr, "%d %c", &nonmemops[numc], &opertype[numc]) > 0) {
        if (opertype[numc] == 'R') {
          if (sscanf(newstr, "%d %c %Lx %Lx %d", &nonmemops[numc],
          ...
          if (opertype[numc] == 'W') {
            if (sscanf(newstr, "%d %c %Lx %d", &nonmemops[numc],
                       &opertype[numc], &addr[numc], &domain_id[numc]) < 1) {
              printf("Panic.  Poor trace format.\n");
              return -3;
            }
          } else {
  ...
  while (!expt_done) {
    ...
    for (numc = 0; numc < NUMCORES; numc++) {
      if (!ROB[numc]
               .tracedone) { /* Try to fetch if EOF has not been encountered. */
        num_fetch = 0;
        while ((num_fetch < MAX_FETCH) && (ROB[numc].inflight != ROBSIZE) &&
          ...
          if (nonmemops[numc]) { /* Have some non-memory-ops to consume. */
            ...
            fetched[numc]++;
            num_fetch++;
          } else { /* Done consuming non-memory-ops.  Must now consume the
                      memory rd or wr. */
            if (opertype[numc] == 'R') {
              addr[numc] =
                  addr[numc] +
                  (long long int)((long long int)prefixtable[numc]
                                  << (ADDRESS_BITS -
                                      log_base2(
                                          NUMCORES))); // Add MSB bits so each
                                                       // trace accesses a
                                                       // different address
                                                       // space.
              ...
              ROB[numc].domain_id[ROB[numc].tail] = domain_id[numc];
              ...
              if (SECURED == 0){
                ...
                //lat = read_matches_write_or_read_queue_secure(addr[numc], domain_id[numc]);
                // if (lat) {
                //   ROB[numc].comptime[ROB[numc].tail] =
                //       CYCLE_VAL + lat + PIPELINEDEPTH;
                // } else {
                 insert_read_secure(addr[numc], CYCLE_VAL, numc, ROB[numc].tail,
                                     instrpc[numc], domain_id[numc]);
                // }
              }
              ...
              if (opertype[numc] == 'W') {
                ...
                if (SECURED == 0){
                  ...
                  for (int c = 0; c < NUM_CHANNELS; c++) {
                    if (write_queue_length[c] == WQ_CAPACITY) {
                      writeqfull = 1;
                      printf("Write Queue Full\n");
                      break;
                    }
                  }
                // secure version
                } else {
                  //similar to above but only modify our own domain!
                  // if (!write_exists_in_write_queue_secure(addr[numc], domain_id[numc])) {
                    insert_write_secure(addr[numc], CYCLE_VAL, numc, ROB[numc].tail,
                                domain_id[numc]);
                  // }
                  //not sure what to do here! but the two sims i ran never actually hit this?!?
                  for (int c = 0; c < NUM_CHANNELS; c++) {
                    if (secure_write_queue_length[c][domain_id[numc]] == WQ_CAPACITY) {
                      writeqfull = 1;
                      break;
                    }
                  }
      ...
      if(num_done == NUMCORES){
        for (numch = 0; numch < NUM_CHANNELS; numch++) {
          for (int d = 0; d < DOMAIN_COUNT; d++){
            if (secure_write_queue_length[numch][d])
              break;
          }
        }
        if (numch == NUM_CHANNELS)
    ...
    // printf("Cycle: %lld\n", CYCLE_VAL);
    // for (numc=0; numc < NUMCORES; numc++) {
    //  printf("C%d: Inf %d : Hd %d : Tl %d : Comp %lld : type %c : addr %llx : TD%d\n", numc, ROB[numc].inflight, ROB[numc].head, ROB[numc].tail,
  ...
  if (NUM_CHANNELS == 4) { /* Assuming that this is 4channel.cfg  */
    ...
    printf("Miscellaneous system power = 40 W  # Processor uncore power, disk, "
           "I/O, cooling, etc.\n");
    printf("Processor core power = %f W  # Assuming that each core consumes 10 "
           "W when running\n",
           core_power);
    ...
    printf("Total memory system power = %f W\n", total_system_power / 1000);
    printf(
        "Miscellaneous system power = 10 W  # Processor uncore power, disk, "
        "I/O, cooling, etc.\n"); /* The total 40 W misc power will be split
                                    across 4 channels, only 1 of which is being
                                    considered in the 1-channel experiment. */
    printf(
        "Processor core power = %f W  # Assuming that each core consumes 5 W\n",
