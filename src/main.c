...
int main(int argc, char * argv[])
{
  ...
  for(numc=0; numc<NUMCORES; numc++)
  {
		  ...
		  if (opertype[numc] == 'R') {
		    if (sscanf(newstr,"%d %c %Lx %Lx %d",&nonmemops[numc],&opertype[numc],&addr[numc],&instrpc[numc], &domain_id[numc]) < 1) {
		    ...
		    if (opertype[numc] == 'W') {
		      if (sscanf(newstr,"%d %c %Lx %d",&nonmemops[numc],&opertype[numc],&addr[numc], &domain_id[numc]) < 1) {
		        printf("Panic.  Poor trace format.\n");
		        return -3;
		      }
		    }
	  ...
	  if (nonmemops[numc]) {  /* Have some non-memory-ops to consume. */
	    ...
	    num_fetch++;
		  ...
		  if(lat) {
			...
			insert_read(addr[numc], CYCLE_VAL, numc, ROB[numc].tail, instrpc[numc], domain_id[numc]);
		  }
	      }
			...
			insert_write(addr[numc], CYCLE_VAL, numc, ROB[numc].tail, domain_id[numc]);
