...
typedef struct opt_queue{
  int domain_id;
  struct opt_queue *next;
} opt_queue_t;

typedef struct fs_data {
  long long last_req_issue_cycle;
  int domain_turn;
  int bank_turn;
  int domain_zero_starter;
  int deadtime;
  int next_op;
  optype_t buffer[DOMAIN_COUNT][100]; 
  int pointer;
  opt_queue_t *domain_reads;
  opt_queue_t *domain_writes;
} fs_data_t;
...
// Each channel has a set of queues per domain
request_t *domain_queues[MAX_NUM_CHANNELS][DOMAIN_COUNT];
...
//read/write queue conuters for secure system
long long int secure_read_queue_length[MAX_NUM_CHANNELS][DOMAIN_COUNT];
long long int secure_write_queue_length[MAX_NUM_CHANNELS][DOMAIN_COUNT];
...
// find if there is a matching write request secure version
int read_matches_write_or_read_queue_secure(long long int physical_address,
                                            int domain_id);

// find if there is a matching request in the write queue
int write_exists_in_write_queue(long long int physical_address);

// find if there is a matching request in the write queue, secure version
int write_exists_in_write_queue_secure(long long int physical_address,
                                       int domain_id);
